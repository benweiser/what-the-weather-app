...
async function getRequest(query, config, storeInSession, storageId) {
  console.log('get response fired');
  try {
    ...
    if (storeInSession && storageId) {
      console.log('storage sesion and id set ');
...
const getCachedAjax = (query, config, storageId) => {
  ...
  const parsedResponse = JSON.parse(sessionStorage.getItem(identifier));
  console.log('parsed response', parsedResponse);
  console.log('is expired', isExpired(parsedResponse, EXPIRE_TIME));
  return isExpired(parsedResponse, EXPIRE_TIME)
    ? parsedResponse
