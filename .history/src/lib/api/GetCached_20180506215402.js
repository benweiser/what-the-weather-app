...
const isExpired = (expireTime, response) =>
  new Date().getTime - response.timestamp > EXPIRE_TIME;
 ...
 */
function getCachedAjax(query, config, storageId) {
  ...
  if (sessionStorage.getItem(identifier) === null) {
    ...
    return isExpired ? getRequest(query, config, identifier) : parsedResponse;
  }
